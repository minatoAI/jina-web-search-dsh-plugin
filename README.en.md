...
- **test** Full suite: **169 tests (168 pass, 1 opt-in skip, 0 fail)**, with four new `jina_pdf` cases (image blocks with names/bytes, the no-store fallback, the `maxImages` cap, and one refused crop not losing the extraction).
