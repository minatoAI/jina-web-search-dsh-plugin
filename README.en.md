...
Every read also sends three zero-side-effect parameters: `X-Preset: agent` (the vendor's preset for AI agents — the docs state a preset only fills options the caller did not set explicitly, so it never overrides an explicit parameter), `X-Base: final` (resolve relative links against the post-redirect URL) and `X-Timeout: 120` (a slow-page safety net matched to the client's own 120 s ceiling). **Exception**: the attempt that carries a target selector runs with a 30 s server patience and a 45 s client ceiling — `X-Target-Selector` implies `X-Wait-For-Selector`, and "wait forever when it does not match" used to collide with the client ceiling and surface as a network timeout (0.12.1).
...
- `test/reader-headers.test.js`: drives the host half through a fake Cordis context and **decodes the network helper's stdin**, asserting the Reader headers `jina_read` really sends — the fixed parameters (`X-Preset: agent` / `X-Base: final`), `X-Timeout` and the client ceiling split into the 30 s/45 s selector pair and the 120 s/120 s whole-page pair, the image policy, the selector group's three fallback shapes (short body / 422 / timeout) and "an explicit empty `targetSelector` reads the whole page", the OCR switch and `X-Page`, zero requests when no key can be resolved, the three alt-text constraints (opt-in, key-gated, mutually exclusive with OCR), JSON-envelope unwrapping with usage, verbatim fallback for an unparsable body, the schema's field declarations (all seven fields carry `meta.volatile`), the validator's tolerant surface, and "a save reaches the next call".
