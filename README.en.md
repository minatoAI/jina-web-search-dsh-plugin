...
- **feat** **`jina_pdf` now returns the extracted figures/tables as images.** The raw payload showed that `extract-pdf` answers each detected "float" (LaTeX's word for figures/tables/equations) with only `type / number / page / caption / image(base64 PNG) / width / height` — **no text field at all**, and the caption is the constant placeholder `"Table (detected)"`. The image *is* the payload, so the old formatter printing only an inventory and dropping 1.2 MB of images left the tool useless. Now: base64 → `Uint8Array` → `ctx.attachments.saveImage()` → `{ type: 'image', attachment }` content blocks (the same contract the built-in `read_image` uses), so **the model can actually look at them**. `maxImages` bounds how many ride along (default 5); a crop the store refuses is reported in `[Notes]` instead of losing the whole extraction. **Verified live**: 11 tables from arXiv `2601.21337`, the first two decoding to `137,80,78,71,13,10,26,10` (the PNG magic number).
- **change** **ReaderLM-v2 removed** (card option `useReaderLm`, the `readerlm` parameter, and its code/tests/docs). Measured: it tied the plain extractor on ordinary pages; it could not carry the selector group (422 `No content available`); the vendor positions it as "you already have the HTML → markdown/JSON", and the one irreplaceable capability (HTML→JSON) has no use here; and on a long formula-heavy page it echoed the raw HTML and blew past the 1.5 MB transport cap. A related official-doc staleness: the model card's `x-engine: readerlm-v2` header **does nothing** on the live API (1x billing, plain-extractor output) — only `X-Respond-With` really engages the model.
- **fix** **A truncated response is no longer misreported**: past the 1.5 MB transport cap the plugin now says so and names the spilled body, instead of handing the truncated slice to `JSON.parse` and reporting `helper output not parseable` beside a garbled snippet.
...
- **test** Full suite: **171 tests (170 pass, 1 opt-in skip, 0 fail)**, with six new `jina_pdf` cases (image blocks with names/bytes, the no-store fallback, the `maxImages` cap, one refused crop not losing the extraction, a text-only route storing nothing, and an unresolvable route still attaching).
...
- **change** **`jina_pdf` now asks whether the current model can see images**: it applies the harness's own gate (the same route resolution and `inputModalities` check `read_image` uses) — a **resolved** text-only route gets no attachments, with the reason in `[Notes]` (attaching would only fill durable history with blocks the harness replaces by a placeholder on every request); an **unresolvable** route still attaches, because the harness projects images to text itself and the plugin must not drop them on its behalf.
- **test** Full suite: **171 tests (170 pass, 1 opt-in skip, 0 fail)**, plus a **live end-to-end verification** against the real API through a real subprocess seam: `jina_read_pdf` asked for pages 1-3 of a one-page PDF reads page 1 only, stops with "page 2 repeated page 1", and marks its provenance; `jina_read` with `readerlm` returns correctly.
...
| `jina_read` | `jina read` | Read a web page as clean markdown, with CSS target selectors and chrome filtering (see below). **PDFs always use the plain extractor** — verbatim and ~40x cheaper than OCR |
...
| `jina_pdf` | `jina pdf` | Extract the figures/tables/equations of a PDF (arXiv ID supported) and **attach each extracted crop to the result as an image**, so the model can look at it. This is a VISUAL extractor: it returns no body text, no table content and no LaTeX, and its caption is a placeholder — use `jina_read_pdf` for words. Only allowlisted sources are accepted (arXiv works, most hosts answer 400) |
...
- `test/reader-headers.test.js`: drives the host half through a fake Cordis context and **decodes the network helper's stdin**, asserting the Reader headers `jina_read` really sends — the three fixed parameters (`X-Preset: agent` / `X-Base: final` / `X-Timeout: 120`), the image policy, the selector group and its "empty result" retry, jina_read never asking for a model pipeline at all, zero requests when no key can be resolved, the three alt-text constraints (opt-in, key-gated, mutually exclusive with a model pipeline), the `jina_read_pdf` page loop with its end-of-document stop, the non-PDF guard and its escape hatch, the `jina_pdf` crops becoming image blocks (and the graceful path when no store is mounted), JSON-envelope unwrapping with usage, verbatim fallback for an unparsable body, the schema's field declarations (all eight fields carry `meta.volatile`), the validator's tolerant surface, and "a save reaches the next call".
