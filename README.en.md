...
- **fix** **Fixed the card's reader-options block and both save buttons being greyed out on current harnesses** (reproduced from a user screenshot): dsh `601d6761e4` removed `settings.register(ns, schema)` — the settings namespace is now the **Loader composition entry id**, and a plugin declares its fields by exporting a schemastery-shaped `Config` (field nodes carrying `meta.volatile`) and receiving `apply(ctx, config)`. The old code called the now-missing `sctx.settings.register`, the throw was swallowed by the plugin's try/catch, so `jina-tools` never appeared in `settings.describe()` → the card's `phase` stayed `'unavailable'` → every control `disabled`. **Not CSS, not permissions, not a read-only profile** — and the same root cause made saving impossible.
- **fix** **Saves apply immediately**: each field is a cross-copy-safe volatile reference (`Symbol.for('cosmokit.volatile.write')`); the harness only writes into those references, so `fiber.config` keeps its identity, nothing remounts, and the plugin re-reads per operation — the same contract the API key already had.
- **fix** Per-field `meta.volatile` (never a whole-config volatile node, which would make `strip()` drop undeclared keys); `validate` returns a plain object synchronously and tolerates `undefined` / mangled values (`describe()` is shared by the whole Plugins page, so a throw breaks every card); `toJSON()` returns fresh nodes each call (`plainSchema()` deletes `meta.volatile`).
- **change** The host half no longer injects `settings`; the primer payload's `settingsError` became a `settingsLive` health check.
- **test** All four affected test files migrated to the new seam, plus an end-to-end "a save reaches the next call" regression; full suite 104 tests (103 pass, 1 opt-in skip).
